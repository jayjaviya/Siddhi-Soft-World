...
/**
 * Orb Component - A high-end, shader-based 3D sphere
 * using WebGL via the OGL library.
 */
export default function Orb({
  ...
  const frag = /* glsl */ `
    ...
    vec4 draw(vec2 uv) {
      ...
      // Noise scales with hover to "start animating" without jumps
      float n0 = snoise3(vec3(uv * noiseScale, noiseTime)) * 0.5 + 0.5;
      ...
      // Light color cycle speeds up on hover
      float cl = cos(ang + lightRot * 2.0) * 0.5 + 0.5;
      
      // CONSTANT ROTATION: The 'a' angle increments smoothly in JS
      float a = lightRot;
    ...
    vec4 mainImage(vec2 fragCoord) {
      ...
      // Wobble only on hover
      uv.x += hover * hoverIntensity * 0.1 * sin(uv.y * 10.0 + noiseTime);
      uv.y += hover * hoverIntensity * 0.1 * sin(uv.x * 10.0 + noiseTime);
